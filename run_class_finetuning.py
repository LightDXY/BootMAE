...
def main(args, ds_init):
    ...
    if float(args.layer_scale_init_value) == 0:
        args.layer_scale_init_value = None
        print ('No Layer Scale')
    model = create_model(
        args.model,
        pretrained=False,
    ...
    if args.finetune:
        ...
        for model_key in args.model_key.split('|'):
            if model_key == 'slip':
                temp_model = checkpoint['state_dict']
                checkpoint_model = {}
                for k, v in temp_model.items():
                    if 'module.visual' in k:
                        k_ = k.replace('module.visual.', '')
                        checkpoint_model[k_] = v
                print("Load slip state_dict by model_key = %s" % model_key)
                break
        ...
        for key in all_keys:
            ...
            if "relative_position_bias_table" in key and model.use_rel_pos_bias:
    ...
    if args.layer_decay < 1.0:
        ...
        decay = list(args.backbone_decay * args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2))
        decay[-1] /= args.backbone_decay
        assigner = LayerDecayValueAssigner(decay)
    else:
        assigner = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_flag = epoch % 5 == 0 or epoch/args.epochs > 0.7 or args.eval_all
        if data_loader_val is not None and test_flag :
            test_stats = evaluate(data_loader_val, model, device)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            if model_ema is not None and (epoch/args.epochs > 0.5 or args.eval_all):
                test_stats = evaluate(data_loader_val, model_ema.ema, device)
                print(f"EMA Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
